...
class Connector_JUnit {
	...
	void MedicaoByCultura() {
		try {
			LinkedList<Medicao> list = b.getMedicoesDaCultura(((Integer) 3).toString());
			for (Medicao m : list)
				System.out.println(
	...
	void getMedicoesCulturaVariavel() {
		try {
			LinkedList<LinkedList<Medicao>> list = b.getMedicoesDaCulturaByVariable(((Integer) 3).toString());
			int i = 0;
